import { K as ensure_array_like, G as escape_html, D as pop, A as push, N as attr_class, E as attr } from "../../../chunks/index.js";
import { u as userStore, h as hasAccessToMission } from "../../../chunks/user.js";
import "dequal";
...
function card($$payload, title, url, iconSrc, hasAcess, classes = "") {
  $$payload.out += `<a${attr_class(`relative overflow-clip h-[200px] w-[150px] p-4 rounded-3xl ${hasAcess ? "hover:scale-105 active:scale-100" : ""} ${classes}`)}${attr("href", hasAcess ? url : void 0)}${attr("aria-disabled", !hasAcess)}>`;
  if (!hasAcess) {
    $$payload.out += "<!--[-->";
    $$payload.out += `<div class="absolute inset-0 bg-silver-800 bg-opacity-50 z-50 flex items-center justify-center"><img src="/lock.png" alt="Mission Locked"></div>`;
  } else {
    $$payload.out += "<!--[!-->";
  }
  $$payload.out += `<!--]--> <div class="relative h-full z-0"><img class="absolute top-2"${attr("src", iconSrc)} alt="img"> <h4 class="absolute bottom-0 !font-normal text-white">${escape_html(title)}</h4></div></a>`;
}
function _page($$payload, $$props) {
  ...
  const each_array = ensure_array_like(missions);
  $$payload.out += `<div class="w-full grid grid-cols-1 place-items-center place-content-evenly h-full px-32 gap-8"><div class="cloud w-[30vw] p-6 rounded-[8rem] bg-secondary-100 border-3 border-black"><div class="flex gap-4 place-content-center min-w-[80%]"><div class="flex place-items-center gap-2"><img class="size-6" src="/Score.png" alt="Skor"> <p>${escape_html(user.score)}</p></div> <div class="flex place-items-center gap-2"><img class="size-6" src="/trophy.png" alt="Trophies"> <p>${escape_html(user.badges.length)}</p></div></div></div> <!--[-->`;
  for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
    let mission = each_array[$$index];
    $$payload.out += `<div class="bg-secondary-100 p-8 rounded-3xl w-full flex flex-col gap-4"><div class="flex w-full justify-between"><h2 class="self-start !font-normal">${escape_html(mission.title)}</h2> <h2 class="self-end">🎮</h2></div> <div class="flex w-full gap-4">`;
    card($$payload, "Tonton dan pelajari", `/standard-pembelajaran/${mission.id}/notes`, "/Learning Bear.png", true, "bg-calm-green");
    $$payload.out += `<!----> `;
    card($$payload, "Permainan", `/standard-pembelajaran/${mission.id}/games`, "/Learning Elephant.png", hasAccessToMission(mission.id), "bg-calm-purple");
    $$payload.out += `<!----></div></div>`;
  }
  $$payload.out += `<!--]--></div>`;
  pop();
